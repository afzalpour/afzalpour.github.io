(function(g){
  ...
  function consumeAuthCallback(){
    ...
    const expires_at=Number(p.get('expires_at')||0)||Math.floor(Date.now()/1000)+expires_in;
    const s={access_token,refresh_token,expires_in,expires_at,token_type:p.get('token_type')||'bearer'};
    saveSession(s);
    const type=p.get('type')||'';
    history.replaceState(null,document.title,location.pathname+location.search);
    return {type,session:s};
  }
  async function requestPasswordReset(email,redirectTo){
    const rt=redirectTo||cfg.authRedirectUrl||location.origin+location.pathname;
    return raw(`/auth/v1/recover?redirect_to=${encodeURIComponent(rt)}`,{method:'POST',body:{email}});
  }
  async function updatePassword(password){
    const t=await token(); if(!t)throw new Error('AUTH_REQUIRED');
    return raw('/auth/v1/user',{method:'PUT',token:t,body:{password}});
  }
  ...
  g.AvanCloud={cfg,SESSION_KEY,session,saveSession,consumeAuthCallback,requestPasswordReset,updatePassword,signup,login,logout,user,select,insert,update,remove,rpc,token};
})(window);
