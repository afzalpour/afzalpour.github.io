...
const sw=read('sw.js');
const cacheVersion=sw.match(/avan-staging-rc1-v(\d+)-[a-z0-9-]+/i);
assert.ok(cacheVersion&&Number(cacheVersion[1])>=119,'Module 7 requires Staging cache v119 or newer.');
assert.ok(sw.includes('./src/intelligence/smart-procurement-spend-foundation.js'));assert.ok(sw.includes('./src/ui/intelligence/smart-procurement-spend-view.js'));
