...
const base = {
  ...
  taxSettings: { tax_enabled: true, taxpayer_type: 'legal', tax_identifier: 'TAX-ID', taxpayer_memory_id: 'MEMORY-ID', e_invoice_enabled: true, default_rule_version_id: 'rule-1' },
  taxRules: [{ id: 'rule-1', name_fa: 'قاعده فعال', effective_from: '2026-03-21', effective_to: '2027-03-20', standard_vat_rate: '10.0000', status: 'active', source_title: 'منبع نسخه‌دار', source_reference: 'REF-1' }],
...
const blocked = buildIranComplianceRadar({ ...base, invoiceLines: [{ id: 'line-x', invoice_id: 'inv-1', line_no: 1, item_id: 'item-1', tax_profile_id: null, tax_rule_version_id: null, tax_treatment: null }] });
...
for (const table of ['workspace_tax_settings', 'tax_profiles', 'inventory_items', 'fiscal_years', 'fiscal_periods', 'invoices', 'invoice_lines']) assert.ok(service.includes(`'${table}'`), `Service must read ${table}.`);
...
assert.ok(Number(cacheVersion[1]) >= 118, 'Module 5 requires Staging cache v118 or newer.');
