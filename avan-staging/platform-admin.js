...
function option(value, current, label, disabled = false) {
...
function registryLabel(value) {
  return ({ ok:'سالم', missing_owner:'مالک ثبت نشده', missing_owner_user:'حساب مالک موجود نیست' })[value] || 'نیازمند بررسی';
}

function renderKpis(data = {}) {
  const items = [
    ['کل شرکت‌ها', data.companies_total], ['فعال', data.companies_active], ['تعلیق', data.companies_suspended],
    ['کاربران', data.users_total], ['پشتیبانی فعال', data.active_support_sessions], ['درخواست پشتیبانی باز', data.support_open],
    ['عضویت فعال', data.active_memberships], ['مدیران سامانه', data.platform_admins_active]
  ];
...
function supportControls(company) {
  ...
  if (session) return `<div class="support-box active-support"><strong>پشتیبانی فعال · فقط‌خواندنی</strong><small>انقضا: ${esc(faDate(session.expires_at))}</small><small>${esc(session.reason || '')}</small><div class="support-actions"><a class="ghost small" href="support-viewer.html?session=${encodeURIComponent(session.session_id)}">بازکردن نمایشگر</a><button class="ghost small danger" data-revoke-support="${esc(session.session_id)}">لغو دسترسی</button></div></div>`;
  return `<div class="support-box"><strong>دسترسی پشتیبانی</strong><div class="support-create"><select data-support-duration><option value="15">۱۵ دقیقه</option><option value="30">۳۰ دقیقه</option><option value="60">۶۰ دقیقه</option></select><textarea data-support-reason rows="2" maxlength="500" placeholder="دلیل دسترسی پشتیبانی (حداقل ۱۰ نویسه)"></textarea><button class="ghost small" data-create-support="${esc(company.company_id)}">شروع دسترسی فقط‌خواندنی</button></div></div>`;
}
...
function renderCompanies(filter = '') {
  ...
  companyRows.innerHTML = rows.length ? rows.map(company => {
    ...
    return `<tr data-company-row="${esc(company.company_id)}"><td><div class="company-name"><strong>${esc(company.display_name || company.name || 'شرکت بدون نام')}</strong><small>${esc(company.owner_email || 'مالک نامشخص')}</small></div></td><td><div class="member-limit"><span>${faNumber(company.active_members)} فعال</span><input data-field="member_limit" type="number" min="1" max="10000" value="${Number(company.member_limit || 10)}"></div></td><td><select data-field="status">${selectOptions('status', company.status)}</select><small>${company.access_allowed ? 'دسترسی باز' : 'دسترسی بسته'}</small></td><td><select data-field="plan_code">${selectOptions('plan', company.plan_code || 'core')}</select></td><td><select data-field="onboarding_state">${selectOptions('onboarding', company.onboarding_state || 'completed')}</select></td><td><select data-field="support_state">${selectOptions('support', company.support_state || 'none')}</select></td><td><span class="badge ${ok ? 'ok' : 'bad'}">${esc(registryLabel(company.registry_state))}</span></td><td class="operation-cell"><textarea data-field="reason" rows="2" maxlength="500" placeholder="دلیل تغییر (الزامی)"></textarea><button class="primary small" data-save-company="${esc(company.company_id)}">ثبت عملیات</button><span class="row-status" aria-live="polite"></span>${supportControls(company)}</td></tr>`;
  }).join('') : '<tr><td colspan="8" class="empty">موردی پیدا نشد.</td></tr>';
  bindActions();
}

const AUDIT_ACTION_FA = Object.freeze({
  control_plane_opened: 'ورود به مرکز مدیریت سامانه',
  tenant_operations_updated: 'تنظیمات عملیاتی شرکت تغییر کرد',
  support_session_created: 'دسترسی پشتیبانی ایجاد شد',
  support_session_revoked: 'دسترسی پشتیبانی لغو شد',
  tax_rule_draft_created: 'پیش‌نویس قانون مالیاتی ایجاد شد',
  tax_rule_published: 'نسخه قانون مالیاتی منتشر شد'
});

function renderAudit(rows = []) {
  auditHost.innerHTML = rows.length ? rows.map(row => `<div class="audit-item"><div><strong>${esc(AUDIT_ACTION_FA[row.action] || 'رویداد مدیریتی ثبت شد')}</strong><span>${row.tenant_id ? 'مرتبط با یک شرکت' : 'رویداد سراسری سامانه'}</span></div><time>${esc(faDate(row.created_at))}</time></div>`).join('') : '<div class="empty">هنوز رویدادی ثبت نشده است.</div>';
}
...
async function saveCompany(id, button) {
  ...
  if (reason.length < 5) { status.textContent = 'دلیل حداقل ۵ نویسه الزامی است.'; return; }
  button.disabled = true;
  status.textContent = 'در حال ثبت…';
  try {
    await client.rpc('platform_admin_update_tenant', {
      p_company_id: id,
      p_status: get('status').value,
    ...
    status.textContent = String(error?.message || '').includes('PLATFORM_OWNER_REQUIRED') ? 'آرشیو فقط برای مالک سامانه مجاز است.' : 'عملیات ثبت نشد.';
  } finally { button.disabled = false; }
}

async function createSupport(id, button) {
  const row = button.closest('[data-company-row]');
  const reason = String(row.querySelector('[data-support-reason]')?.value || '').trim();
  const minutes = Number(row.querySelector('[data-support-duration]')?.value || 15);
  const status = row.querySelector('.row-status');
  if (reason.length < 10) { status.textContent = 'دلیل پشتیبانی حداقل ۱۰ نویسه باشد.'; return; }
  button.disabled = true;
  status.textContent = 'در حال ایجاد دسترسی…';
  try {
    const session = await client.rpc('platform_admin_create_support_session', { p_company_id: id, p_duration_minutes: minutes, p_reason: reason });
    status.textContent = 'دسترسی فقط‌خواندنی فعال شد.';
    await load({ logEntry: false });
    if (session?.session_id) window.open(`support-viewer.html?session=${encodeURIComponent(session.session_id)}`, '_blank', 'noopener');
  } catch (error) {
    status.textContent = String(error?.message || '').includes('SUPPORT_SESSION_ALREADY_ACTIVE') ? 'برای این شرکت دسترسی پشتیبانی فعال وجود دارد.' : 'ایجاد دسترسی انجام نشد.';
  } finally { button.disabled = false; }
}

async function revokeSupport(sessionId, button) {
  if (!confirm('دسترسی پشتیبانی فوراً لغو شود؟')) return;
  button.disabled = true;
  try {
    await client.rpc('platform_admin_revoke_support_session', { p_session_id: sessionId, p_reason: 'لغو دستی توسط مدیر سامانه آوان' });
    await load({ logEntry: false });
  } catch { alert('لغو دسترسی انجام نشد.'); }
  finally { button.disabled = false; }
}
...
async function load({ logEntry = true } = {}) {
  ...
  setAuthState('در حال بررسی دسترسی مدیر سامانه…');
  content.hidden = true;
  try {
    const user = await client.user();
    if (!user?.id) { setAuthState('ابتدا در آوان وارد حساب کاربری شوید.', 'error'); return; }
    const me = await client.rpc('platform_admin_me', {});
    if (!me?.authorized) { setAuthState('این حساب مجوز مدیریت سامانه را ندارد.', 'error'); return; }
    ...
    const [overview, list, audit] = await Promise.all([
      ...
      client.rpc('platform_admin_audit', { p_limit: 50 })
    ]);
    ...
    setAuthState(`دسترسی تأیید شد — ${platformRole === 'platform_owner' ? 'مالک سامانه' : 'مدیر سامانه'}. دسترسی پشتیبانی فقط موقت و فقط‌خواندنی است.`, 'ok');
    window.dispatchEvent(new CustomEvent('avan:platform-admin-refreshed'));
  } catch (error) {
    console.error(error);
    setAuthState('بارگذاری مرکز مدیریت سامانه انجام نشد.', 'error');
