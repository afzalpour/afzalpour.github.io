...
function effectiveRule(rules, ruleCode, date) {
  return rules
...
function profileRate(profile, rules, date) {
...
async function refreshInvoice(form) {
  ...
  for (const row of form.querySelectorAll('[data-invoice-line]')) {
    ...
    for (const option of select.options) {
      ...
      const suffix = profile.treatment === 'exempt' ? 'معاف' : profile.treatment === 'zero' ? 'نرخ صفر' : `${Number(resolved.rate).toLocaleString('fa-IR', { maximumFractionDigits: 4 })}٪`;
      option.textContent = `${profile.name_fa} — ${suffix}`;
    }
    if (!rowUsed(row)) continue;
    const base = lineBase(row);
    const profile = data.profiles.find(item => item.id === select.value);
    const resolved = profileRate(profile, data.rules, date);
    const rowTax = profile ? (calculateVatAmount({ taxableAmount: base, rate: resolved.rate }) ?? 0n) : 0n;
    subtotal += base;
    tax += rowTax;
    const note = row.querySelector('[data-rc15-line-tax-note]');
    if (note) note.textContent = profile
  ...
  form.dataset.rc15TaxTotal = tax.toString();
  form.dataset.rc15GrandTotal = (subtotal + tax).toString();
}
...
function bindInvoice(form) {
  if (!form || form.dataset.rc15C1Bound === '1') return;
  form.dataset.rc15C1Bound = '1';
  const schedule = () => setTimeout(() => void refreshInvoice(form), 0);
...
async function apply() {
  ...
  try {
    ...
    bindInvoice(document.getElementById('invoiceForm'));
    await refreshInvoice(document.getElementById('invoiceForm'));
  } catch (error) {
    console.warn('[Avan tax date-aware]', error);
  } finally {
    busy = false;
  }
}

Lifecycle.use('tax-c1:date-aware-and-singleton', () => apply(), { priority: 260 });
window.addEventListener('avan:company-context-changed', () => { cache = null; Lifecycle.schedule('tax-c1-company'); });
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => Lifecycle.schedule('tax-c1-ready'), { once: true });
else Lifecycle.schedule('tax-c1-ready');
