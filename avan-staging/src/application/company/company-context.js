...
export function createCompanyContext({client,listWorkspaces,globalObject=window,activeKey=DEFAULT_ACTIVE_KEY}={}){
  ...
  function sessionUser(){try{return client.session?.()?.user||null}catch{return null}}
  ...
  async function ensure(){const hydrated=sessionUser();const uid=hydrated?.id||null;if(state.ready&&uid&&uid===state.userId)return snapshot();if(state.ready&&!uid&&!state.userId)return snapshot();if(uid&&uid!==state.userId){await refresh({force:true});return snapshot();}if(!state.ready){await refresh({force:true});return snapshot();}const user=await client.user();const remoteUid=user?.id||null;if(remoteUid!==state.userId)await refresh({force:true});return snapshot()}
