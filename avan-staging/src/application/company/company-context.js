...
function cloneState(state) {
  return Object.freeze({
    ...
    active_company: cloneCompany(state.companies.find(company => company.id === state.activeId) || null)
  });
}
...
export function createCompanyContext({ client, listWorkspaces, globalObject = window, activeKey = DEFAULT_ACTIVE_KEY } = {}) {
  ...
  function reorderById(rows, activeId) {
    const items = normalizeRows(rows); if (!activeId) return items;
    const index = items.findIndex(item => item.id === activeId); if (index <= 0) return items;
    return [items[index], ...items.slice(0,index), ...items.slice(index+1)];
  }
  function resolveFullActiveId(rows) {
    const items = allowedRows(rows); const preferred = storedId();
    if (preferred && items.some(item => item.id === preferred)) return preferred;
    if (preferred) persistId(null);
    if (items.length === 1) { persistId(items[0].id); return items[0].id; }
    return null;
  }
  ...
  async function enrich(workspace) {
    ...
    let profile = null;
    if (!role && workspace.access_allowed !== false) {
      try { role = await client.rpc('workspace_role',{ wid:workspace.id }) || ''; } catch {}
    }
    if (!workspace.display_name && workspace.access_allowed !== false) {
      try { profile = await client.rpc('get_workspace_print_profile',{ wid:workspace.id }); } catch {}
    }
    return Object.freeze({
      ...workspace,
      role,
      status: workspace.status || 'active',
      access_allowed: workspace.access_allowed !== false,
      display_name: String(workspace.display_name || profile?.display_name || workspace.name || 'شرکت بدون نام').trim(),
      legal_name: String(workspace.legal_name || profile?.legal_name || '').trim()
    });
  }
  ...
  async function refresh({ force=false }={}) {
    ...
    refreshPromise = (async()=>{
      ...
      try {
        ...
        if(!user?.id){ Object.assign(state,{ready:true,userId:null,activeId:null,selectionRequired:false,companies:[]}); return cloneState(state); }
        ...
        const allowedCount=companies.filter(c=>c.access_allowed).length;
        Object.assign(state,{userId:user.id,companies,activeId,selectionRequired:allowedCount>1&&!activeId,ready:true});
  ...
  async function selectCompany(companyId,{emit=true}={}){
    const id=String(companyId||'').trim(); if(!id) throw new Error('COMPANY_REQUIRED');
    await ensure();
    const target=state.companies.find(c=>c.id===id);
    if(!target){ persistId(null); state.activeId=null; state.selectionRequired=state.companies.filter(c=>c.access_allowed).length>1; throw new Error('COMPANY_ACCESS_REQUIRED'); }
    if(target.access_allowed===false){ if(storedId()===id) persistId(null); throw new Error(target.status==='archived'?'COMPANY_ARCHIVED':'COMPANY_SUSPENDED'); }
    persistId(id); state.activeId=id; state.selectionRequired=false; state.companies=[target,...state.companies.filter(c=>c.id!==id)];
    if(emit){ const detail={company:cloneCompany(target),company_id:id}; globalObject.dispatchEvent(new CustomEvent('avan:company-context-changed',{detail})); globalObject.dispatchEvent(new CustomEvent('avan:workspace-changed',{detail:{workspace_id:id}})); }
    return cloneCompany(target);
  }

  function clearSelection({emit=true}={}){ persistId(null); state.activeId=null; state.selectionRequired=state.companies.filter(c=>c.access_allowed).length>1; if(emit) globalObject.dispatchEvent(new CustomEvent('avan:company-context-cleared')); }
