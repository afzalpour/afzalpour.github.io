...
'./rc15-final-web-pwa.css','./src/ui/settings/settings-layout-v2.js','./src/ui/settings/workspace-access-settings.js','./src/ui/settings/support-access-settings.js','./src/ui/einvoice/einvoice-preflight-ui.js','./src/ui/tax/invoice-item-optional.js','./src/ui/invoices/invoice-base-save-normalizer.js','./src/ui/settlement/settlement-money-presentation.js',
...
self.addEventListener('install',event=>{const freshAssets=ASSETS.map(asset=>new Request(new URL(asset,self.registration.scope),{cache:'reload'}));event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(freshAssets)).then(()=>self.skipWaiting()));});
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(key=>key.startsWith(CACHE_PREFIX)&&key!==CACHE).map(key=>caches.delete(key)));await self.clients.claim();const clients=await self.clients.matchAll({type:'window'});await Promise.allSettled(clients.map(client=>client.navigate(client.url)));})());});
async function networkFirst(request){try{const freshRequest=new Request(request,{cache:'reload'});const response=await fetch(freshRequest);if(response.ok){const cache=await caches.open(CACHE);await cache.put(request,response.clone());}return response;}catch(error){const cached=await caches.match(request);if(cached)return cached;if(request.mode==='navigate'){const shell=await caches.match('./index.html')||await caches.match('./');if(shell)return shell;}throw error;}}
