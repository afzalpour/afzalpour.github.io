...
function isSettingsPage() { return String(pageTitle?.textContent || '').trim() === 'تنظیمات'; }
function isAuditAdminRole() { return workspaceRole === 'owner' || workspaceRole === 'manager'; }
...
function cardShell() {
  return `<section class="section card avan-audit-card" data-avan-operational-audit><div class="section-head avan-audit-head"><div><h2>گزارش فعالیت</h2><span class="muted">آخرین رویدادهای شرکت فعال، فقط به‌صورت خواندنی</span><small class="muted" id="avanAuditScopeNote"></small></div><button type="button" class="ghost" id="avanAuditRefresh">به‌روزرسانی</button></div><div class="avan-audit-toolbar"><label for="avanAuditFilter">فیلتر</label><select id="avanAuditFilter"><option value="all">همه فعالیت‌ها</option><option value="accounting">حسابداری و دوره مالی</option><option value="documents">اسناد هوشمند</option><option value="settings">تنظیمات</option></select><span class="muted" id="avanAuditStatus"></span></div><div class="avan-audit-list" id="avanAuditList"><div class="loading">در حال خواندن گزارش فعالیت…</div></div></section>`;
}
...
function paint(category = 'all') {
  const list = document.getElementById('avanAuditList'); const status = document.getElementById('avanAuditStatus');
  if (!list || !status) return;
  const visible = filteredRows(category); status.textContent = `${visible.length.toLocaleString('fa-IR')} رویداد`;
  list.innerHTML = visible.length ? visible.map(rowHtml).join('') : '<div class="muted avan-audit-empty">رویدادی برای این فیلتر وجود ندارد.</div>';
}
async function resolveContext() {
  currentUser = await cloud.user(); if (!currentUser?.id) throw new Error('AUTH_REQUIRED');
  const state = await companyContext.ensure(); workspace = state.active_company || null; if (!workspace?.id) throw new Error('COMPANY_REQUIRED');
...
async function loadAudit({ forceContext = false } = {}) {
  ...
  const refresh = document.getElementById('avanAuditRefresh'); const status = document.getElementById('avanAuditStatus');
  if (refresh) refresh.disabled = true; if (status) status.textContent = 'در حال به‌روزرسانی…';
  try {
    if (forceContext || !workspace?.id || !currentUser?.id || !workspaceRole) { if (forceContext) await companyContext.refresh({ force:true }); await resolveContext(); }
    applyRoleVisibility();
    ...
    paint(document.getElementById('avanAuditFilter')?.value || 'all');
  } catch (error) {
    console.warn('[Avan audit] load failed', error);
    const list = document.getElementById('avanAuditList'); if (list) list.innerHTML = '<div class="error-box">خواندن گزارش فعالیت انجام نشد. دوباره تلاش کنید.</div>';
    if (status) status.textContent = 'خطا در دریافت';
  } finally { loading = false; if (refresh?.isConnected) refresh.disabled = false; }
}
function bindCard() {
  const refresh = document.getElementById('avanAuditRefresh'); const filter = document.getElementById('avanAuditFilter');
  if (refresh) refresh.onclick = () => loadAudit({ forceContext:true }); if (filter) filter.onchange = () => paint(filter.value);
}
function ensureCard() {
  if (!content || !isSettingsPage() || content.querySelector('[data-avan-operational-audit]')) return;
  content.insertAdjacentHTML('beforeend', cardShell()); bindCard(); loadAudit({ forceContext:true });
}
function scheduleEnsure() { if (scheduled) clearTimeout(scheduled); scheduled = setTimeout(() => { scheduled = null; ensureCard(); }, 40); }
if (content) { const observer = new MutationObserver(scheduleEnsure); observer.observe(content, { childList:true }); }
window.addEventListener('avan:company-context-changed', () => { workspace = null; workspaceRole = ''; rows = []; scheduleEnsure(); });
scheduleEnsure();
