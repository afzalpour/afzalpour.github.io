...
const MONEY_INPUT_SELECTOR = [
  ...
  'input[name="v60_amount"]'
].join(',');

const FINANCIAL_PAGES = new Set(['داشبورد', 'فاکتورها', 'اسناد حسابداری', 'گزارش‌ها']);

function currentUnit() {
  return window.AVAN_MONEY_DISPLAY_UNIT === UNIT_RIAL ? UNIT_RIAL : UNIT_TOMAN;
}
...
function parseInteger(value) {
  const cleaned = toLatinDigits(value).replace(/[٬,\s]/g, '').replace(/[^0-9-]/g, '');
...
function canonicalFromRendered(amount, renderedLabel) {
...
function displayFromCanonical(canonical, unit = currentUnit()) {
  ...
  return unit === UNIT_RIAL ? canonical * 10n : canonical;
}
...
function compactTextNode(node) {
  ...
  const matches = [...source.matchAll(MONEY_PATTERN)];
  ...
  for (const match of matches) {
    ...
    const canonical = canonicalFromRendered(match[1], match[2]);
    if (canonical === null) fragment.append(document.createTextNode(match[0]));
    else {
...
function compactNumericRoot(root = document) {
  if (root instanceof Text) return compactTextNode(root);
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      MONEY_PATTERN.lastIndex = 0;
      return MONEY_PATTERN.test(node.nodeValue || '') ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT;
    }
  });
  const nodes = [];
...
function refreshCompactAmounts(unit = currentUnit()) {
  document.querySelectorAll('.money-compact[data-money-canonical]').forEach(element => {
    let canonical;
    try { canonical = BigInt(element.dataset.moneyCanonical); } catch { return; }
