...
const version = "4.3.0-forecast-final-gated"
...
type EvidenceSummary struct {
	...
	ForecastableCleanCases        int     `json:"forecastable_clean_cases"`
	IndependentAnchorWindows      int     `json:"independent_anchor_windows"`
	BaselineMeanAPEPct            float64 `json:"baseline_mean_ape_pct"`
	BestIndividualModel           string  `json:"best_individual_model"`
	BestIndividualMeanAPEPct      float64 `json:"best_individual_mean_ape_pct"`
	BestIndividualAdjustedP       float64 `json:"best_individual_adjusted_p"`
	LatestWindowBaselineMeanAPE   float64 `json:"latest_window_baseline_mean_ape_pct"`
	LatestWindowBestModel         string  `json:"latest_window_best_model"`
	LatestWindowBestModelMeanAPE  float64 `json:"latest_window_best_model_mean_ape_pct"`
	WalkForwardEnsemblesRejected  bool    `json:"walk_forward_ensembles_rejected"`
	Verdict                       string  `json:"verdict"`
}

func evidenceSummary() EvidenceSummary {
	return EvidenceSummary{
		CalibrationVersion: "v4.2.1-90case-20260925",
		RequestedCases: 90,
		CompletedCases: 88,
		CleanCases: 70,
		ForecastableCleanCases: 68,
		IndependentAnchorWindows: 3,
		BaselineMeanAPEPct: 3.4717958216050313,
		BestIndividualModel: "MACD/EMA",
		BestIndividualMeanAPEPct: 3.2116352378331805,
		...
		WalkForwardEnsemblesRejected: true,
...
type BatchReport struct {
	...
	FinalForecastGate string           `json:"final_forecast_gate"`
...
type Report struct {
	...
	GeneratedAt            string          `json:"generated_at"`
	FinalForecastGate      string          `json:"final_forecast_gate"`
	FinalForecastEnabled   bool            `json:"final_forecast_enabled"`
	Evidence               EvidenceSummary `json:"evidence"`
}
...
func buildReport(hist []Candle, symbol, ins, anchorDate, targetDate, policy string) (Report, error) {
	...
	return Report{
		...
		FinalForecastGate: "NO_EDGE", FinalForecastEnabled: false, Evidence: evidenceSummary(),
	}, nil
}
...
func printReport(r Report) {
	...
	fmt.Println("FINAL FORECAST GATE: NO_EDGE — five models remain diagnostic; no combined price/direction is promoted.")
	fmt.Printf("EVIDENCE clean=%d forecastable=%d windows=%d best=%s mean_APE=%.3f%% adjusted_p=%.3f latest_baseline=%.3f%% latest_best_model=%.3f%%\n",
		r.Evidence.CleanCases, r.Evidence.ForecastableCleanCases, r.Evidence.IndependentAnchorWindows,
		r.Evidence.BestIndividualModel, r.Evidence.BestIndividualMeanAPEPct, r.Evidence.BestIndividualAdjustedP,
		r.Evidence.LatestWindowBaselineMeanAPE, r.Evidence.LatestWindowBestModelMeanAPE)
...
func runBatch(path string) (BatchReport, error) {
	...
	return BatchReport{
		...
		FinalForecastGate:"NO_EDGE", Evidence:evidenceSummary(),
	}, nil
}
...
func printBatch(r BatchReport) {
	...
	fmt.Println("FINAL FORECAST GATE: NO_EDGE — calibration does not justify a production ensemble.")
	fmt.Printf("EVIDENCE clean=%d forecastable=%d windows=%d verdict=%s\n",
		r.Evidence.CleanCases, r.Evidence.ForecastableCleanCases, r.Evidence.IndependentAnchorWindows, r.Evidence.Verdict)
