...
const robustFaTerms=s=>String(s??'—')
  .replace(/challenger/gi,'گزینه آزمایشی')
...
function robustMode(m){return m==='reversal'?'برگشت منفی به مثبت':'شتاب مثبت اولیه'}
function robustRow(r){return `<tr><td>${robustMode(r.hunt_mode)}</td><td>${r.candidate_id||'—'}</td><td>${robustFa(r.validation_trade_dates,0)}</td><td>${robustFa(r.paired_trade_dates,0)}</td><td>${robustFa(r.slice_passes,0)}/۳</td><td>${robustSigned(r.worst_slice_diff)}</td><td>${robustFa(r.bootstrap_reps,0)}</td><td>${robustPct01(r.bootstrap_win_rate)}</td><td>${robustSigned(r.bootstrap_p10_diff)}</td><td>${robustSigned(r.bootstrap_median_diff)}</td><td class="${r.robustness_ready?'ok-text':'warn-text'}">${r.robustness_ready?'پاس':'قفل'}</td><td>${robustFaTerms(r.robustness_reason||'—')}</td></tr>`}
...
async function loadRobustness(){
  ...
  robust$('robustStatus').textContent='در حال دریافت وضعیت پایداری و استحکام…';
  try{
    ...
    if(!pr.ok||!rr.ok||!ur.ok||!sr.ok||!mr.ok||!cr.ok)throw new Error('دریافت وضعیت پایداری/خارج‌ازنمونه ناموفق بود');
    ...
    robust$('oosReason').textContent=robustFaTerms(unlock.unlock_reason||'—');
    ...
    robust$('robustBody').innerHTML=rows.length?rows.map(robustRow).join(''):'<tr><td colspan="12" class="muted">تا انتخاب یک گزینه آزمایشی معتبر، دروازه پایداری محاسبه نمی‌شود.</td></tr>';
    robust$('sliceBody').innerHTML=slices.length?slices.map(sliceRow).join(''):'<tr><td colspan="9" class="muted">زیر‌بازه اعتبارسنجی هنوز داده کافی ندارد.</td></tr>';
    robust$('oosReleaseState').textContent=manifest?'آزاد شده':'قفل';
    ...
    robust$('oosReleaseNote').textContent=robustFaTerms(manifest?.release_note||unlock.unlock_reason||'—');
    ...
    robust$('oosResultBody').innerHTML=oosRows.length?oosRows.map(oosResultRow).join(''):'<tr><td colspan="10" class="muted">بخش خارج‌ازنمونه هنوز آزاد نشده و هیچ نتیجه‌ای تثبیت نشده است.</td></tr>';
    robust$('robustStatus').textContent=`آخرین بررسی: ${new Date().toLocaleTimeString('fa-IR')}`;
  }catch(e){robust$('robustStatus').textContent=e.message||'خطا در دریافت وضعیت پایداری/خارج‌ازنمونه'}
}
