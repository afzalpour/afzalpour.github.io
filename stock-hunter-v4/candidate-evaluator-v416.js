...
function modeLabel(m){return m==='reversal'?'Reversal':'Acceleration'}
function row(c){return `<tr><td>${modeLabel(c.hunt_mode)}</td><td>${c.is_baseline?'Baseline':'Candidate'}</td><td>${fa(c.order_pressure,2)}</td><td>${fa(c.impulse,2)}</td><td>${fa(c.feasibility,2)}</td><td>${fa(c.flow_volume,2)}</td><td>${fa(c.market_context,2)}</td><td>${fa(c.train_selected,0)}</td><td>${fa(c.validation_selected,0)}</td><td>${pct(c.validation_return_3d_pct)}</td><td>${pct(c.validation_mfe_3d_pct)}</td><td>${pct(c.validation_mae_3d_pct)}</td><td>${fa(c.validation_utility,3)}</td><td>${c.eligible_for_selection?fa(c.validation_rank,0):'—'}</td></tr>`}
async function loadEvaluator(){
  ...
  $('evalStatus').textContent='در حال دریافت Candidate Evaluator…';
  try{
    ...
    if(!lr.ok||!sr.ok||!cr.ok||!or.ok)throw new Error('دریافت Candidate Evaluator ناموفق بود');
    ...
    $('selectedCandidates').textContent=selected.length?selected.map(x=>`${modeLabel(x.hunt_mode)}: ${x.candidate_id}`).join(' | '):'هنوز Candidate منتخب وجود ندارد';
    ...
    $('candidateBody').innerHTML=top.length?top.map(row).join(''):'<tr><td colspan="14" class="muted">تا بلوغ Dataset، فقط Candidate Grid ساخته شده و رتبه معتبر محاسبه نمی‌شود.</td></tr>';
    $('evalStatus').textContent=`آخرین بررسی: ${new Date().toLocaleTimeString('fa-IR')}`;
  }catch(e){$('evalStatus').textContent=e.message||'خطا در دریافت Evaluator'}
}
