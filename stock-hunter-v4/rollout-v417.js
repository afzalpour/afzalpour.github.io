...
async function loadRolloutV417(){
  ...
  try{
    const [pr,rr,mr,sr]=await Promise.all([
    ...
    if(!pr.ok||!rr.ok||!mr.ok||!sr.ok)throw new Error('دریافت Forward Shadow ناموفق بود');
    const pol=(await pr.json())[0]||{},ready=(await rr.json())[0]||{},rows=await mr.json();
