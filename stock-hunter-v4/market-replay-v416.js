'use strict';
const R=StockHunterResearchV416;const $=id=>document.getElementById(id);let replayRows=[],journey=null,playTimer=null,index=0;
...
function milestone(label,at,cls=''){return '<div class="timeline-step '+(at?'reached':'pending')+'"><span>'+R.esc(label)+'</span><b>'+(at?R.time(at):'نرسیده')+'</b></div>';}
function renderMilestones(){
 if(!journey){$('replayMilestones').innerHTML='<div class="empty">برای این نماد سفر شکار ثبت نشده است.</div>';return;}
 $('replayMilestones').innerHTML=milestone('کشف شکار',journey.detected_at)+
 '<div class="timeline-arrow">←</div>'+milestone('عبور از صفر',journey.crossed_zero_at)+
 '<div class="timeline-arrow">←</div>'+milestone('رسیدن به +۱٪',journey.crossed_plus1_at)+
 '<div class="timeline-arrow">←</div>'+milestone('رسیدن به +۲٪',journey.crossed_plus2_at)+
 '<div class="timeline-arrow">←</div>'+milestone('رسیدن به +۳٪',journey.crossed_plus3_at);
}
function chart(){
 ...
 const zero=Y(0);
 $('replayChart').innerHTML='<svg viewBox="0 0 960 215" preserveAspectRatio="none"><line class="chart-zero" x1="35" x2="945" y1="'+zero+'" y2="'+zero+'"/><text class="chart-label" x="8" y="'+(zero-4)+'">صفر</text><path class="chart-line" d="'+path+'"/>'+marker+'</svg>';
}
...
function renderTable(){
 $('replayBody').innerHTML=replayRows.length?replayRows.map((x,i)=>'<tr data-i="'+i+'"><td>'+R.time(x.bucket_at)+'</td><td>'+R.fa(x.open_price,0)+'</td><td class="good">'+R.fa(x.high_price,0)+'</td><td class="bad">'+R.fa(x.low_price,0)+'</td><td>'+R.fa(x.close_price,0)+'</td><td class="'+(Number(x.close_change_pct)>=0?'good':'bad')+'">'+R.pct(x.close_change_pct)+'</td><td>'+R.fa(x.max_buy_queue,0)+'</td><td>'+R.fa(x.max_sell_queue,0)+'</td><td>'+R.fa(x.last_volume,0)+'</td><td>'+R.fa(x.sample_count)+'</td></tr>').join(''):'<tr><td colspan="10"><div class="empty">برای این نماد داده‌ای وجود ندارد.</div></td></tr>';
...
function scheduleNext(){
 if(!playTimer)return;const speed=Math.max(1,Number($('replaySpeed').value)||1);
 if(index>=replayRows.length-1){stopPlay();return;}
 playTimer=setTimeout(()=>{index++;renderCurrent();scheduleNext();},900/speed);
}
function togglePlay(){
 if(playTimer){stopPlay();return;}if(!replayRows.length)return;
 if(index>=replayRows.length-1)index=0;$('replayPlay').textContent='⏸ توقف بازپخش';$('replayPlay').classList.add('active');playTimer=true;scheduleNext();
}
async function loadSymbols(){
 ...
 try{
   const rows=await R.api('stock_hunter_market_replay_symbols_v416','select=symbol_id,symbol,bucket_count&trade_date=eq.'+encodeURIComponent(d)+'&order=symbol.asc&limit=2000');
   const old=$('replaySymbol').value;$('replaySymbol').innerHTML=rows.length?rows.map(x=>'<option value="'+R.esc(x.symbol_id)+'">'+R.esc(x.symbol)+' — '+R.fa(x.bucket_count)+' نما</option>').join(''):'<option value="">نمادی برای بازپخش وجود ندارد</option>';
   if(rows.some(x=>String(x.symbol_id)===old))$('replaySymbol').value=old;
   await loadReplay();
...
async function loadReplay(){
 ...
 try{
   [replayRows]=await Promise.all([
     R.api('stock_hunter_market_replay_v416','select=*&trade_date=eq.'+encodeURIComponent(d)+'&symbol_id=eq.'+encodeURIComponent(sid)+'&order=bucket_at.asc&limit=500'),
   ]);
   const jr=await R.api('stock_hunter_hunt_journey_v416','select=channel,trade_date,symbol_id,symbol,detected_at,crossed_zero_at,crossed_plus1_at,crossed_plus2_at,crossed_plus3_at,hunt_state,hunt_score&trade_date=eq.'+encodeURIComponent(d)+'&symbol_id=eq.'+encodeURIComponent(sid)+'&order=detected_at.asc&limit=1');
   journey=jr[0]||null;index=0;$('replaySlider').max=String(Math.max(0,replayRows.length-1));renderTable();renderCurrent();renderMilestones();
   const name=$('replaySymbol').selectedOptions[0]?.textContent?.split(' — ')[0]||'نماد';R.setStatus('بازپخش '+name+' در '+R.jalaliDate(d)+' — '+R.fa(replayRows.length)+' نمای پنج‌دقیقه‌ای','ok');
