...
const promoFaTerms=s=>String(s??'—')
  .replace(/Candidate/gi,'گزینه آزمایشی')
  .replace(/Baseline/gi,'مبنا')
  .replace(/Validation/gi,'اعتبارسنجی')
  .replace(/OOS/g,'خارج‌ازنمونه')
  .replace(/Release/gi,'آزادسازی')
  .replace(/Promotion/gi,'ارتقا')
  .replace(/Dataset/gi,'مجموعه‌داده')
  .replace(/Mode/gi,'سناریو');
function promoStatusFa(s){
  const v=String(s||'').toLowerCase();
  if(v==='proposed')return 'پیشنهاد ثبت‌شده';
  if(v==='approved')return 'تأییدشده';
  if(v==='rejected')return 'ردشده';
  return promoFaTerms(s||'—');
}
...
function promoMode(m){return m==='reversal'?'برگشت منفی به مثبت':m==='acceleration'?'شتاب مثبت اولیه':String(m||'—')}
...
function promoProposalRow(r){return `<tr><td>${promoFa(r.proposal_id,0)}</td><td>${r.target_engine_version||'—'}</td><td>${promoStatusFa(r.proposal_status)}</td><td>${r.created_at?new Date(r.created_at).toLocaleString('fa-IR'):'—'}</td><td class="mono">${r.dataset_fingerprint||'—'}</td><td>${r.proposal_note||'—'}</td></tr>`}
function ensureRolloutLinkV417(){
  ...
  a.className='top-link';a.href='rollout-v417.html';a.dataset.rolloutV417='1';a.textContent='آزمایش سایه پیشرو ۴.۱.۷';
  nav.appendChild(a);
}
async function loadPromotionDecision(){
  ...
  promo$('promoStatus').textContent='در حال دریافت پروتکل تصمیم ارتقا…';
  try{
    ...
    if(!pr.ok||!rr.ok||!ar.ok||!qr.ok)throw new Error('دریافت پروتکل ارتقا ناموفق بود');
    ...
    promo$('promoReady').textContent=ready.proposal_id?'پیشنهاد ثبت‌شده':ready.proposal_ready?'آماده ثبت پیشنهاد':'قفل';
    promo$('promoReady').className=ready.proposal_id||ready.proposal_ready?'warn-text':'ok-text';
    promo$('promoReason').textContent=promoFaTerms(ready.readiness_reason||'—');
    promo$('promoAuto').textContent=pol.auto_promote?'فعال':'خاموش';
    promo$('promoAuto').className=pol.auto_promote?'warn-text':'ok-text';
    promo$('promoThresholds').textContent=`آستانه‌های تثبیت‌شده: حداقل ${promoFa(pol.min_oos_selected_per_mode,0)} انتخاب خارج‌ازنمونه در هر سناریو؛ پوشش حداقل ${promoPctRatio(pol.min_selected_coverage_ratio)} نسبت به مبنا؛ بهبود شاخص کارایی حداقل ${promoSigned(pol.min_utility_lift)}؛ بهبود بازده ۳جلسه‌ای حداقل ${promoSigned(pol.min_return_lift_3d_pct)} واحد درصد؛ افت مجاز MAE حداکثر ${promoFa(pol.max_mae_deterioration_3d_pct,2)} واحد درصد و افت مجاز نرخ بازده مثبت حداکثر ${promoFa(pol.max_positive_rate_deterioration_pp,1)} واحد درصد.`;
    promo$('promoAssessmentBody').innerHTML=assess.length?assess.map(promoAssessmentRow).join(''):'<tr><td colspan="16" class="muted">تا پیش از آزادسازی بخش خارج‌ازنمونه، ارزیابی ارتقا ساخته نمی‌شود.</td></tr>';
  ...
  }catch(e){promo$('promoStatus').textContent=e.message||'خطا در دریافت پروتکل ارتقا'}
}
