...
detailHTML = function(...args){
  const x=args[0];
  let html=detailHTMLBeforeIntegratedV410(...args);
...
function printJalaliDateTimeV416(value){
  if(typeof marketFaDateTimeV416==='function')return marketFaDateTimeV416(value);
...
function printHistoricalDetailV416(x){
  const candles=dailyCandles(x),latest=candles[candles.length-1],prev=candles[candles.length-2];
  const pct=latest&&prev?.close?((latest.close-prev.close)/prev.close*100):null;
  const models=forecastModels(x),ft=forecastTable(models),w=window.open('','_blank','width=1200,height=900');
  if(!w){alert('مرورگر پنجره چاپ را مسدود کرده است. اجازه بازشدن پنجره جدید را برای این سایت فعال کنید.');return false;}
  const tradeDate=latest?.t?new Intl.DateTimeFormat('fa-IR-u-ca-persian-nu-latn',{calendar:'persian',numberingSystem:'latn',timeZone:'Asia/Tehran',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date(latest.t*1000)):'—';
  const metrics=[
    ['تاریخ آخرین داده',tradeDate],['قیمت پایانی',latest?fa(latest.close,0):'—'],['تغییر روزانه',pct==null?'—':fa(pct,2)+'٪'],
    ['بیشترین قیمت',latest?fa(latest.high,0):'—'],['کمترین قیمت',latest?fa(latest.low,0):'—'],['حجم',latest?fa(latest.volume,0):'—']
  ];
  w.document.write(`<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>${esc(x.symbol)} - گزارش روزانه شکارچی سهم</title><style>@page{size:A4 landscape;margin:5mm}*{box-sizing:border-box}body{font-family:Tahoma,"IRANSans",sans-serif;color:#111;margin:0;font-size:7pt}h1{font-size:14pt;margin:0}h2{font-size:9pt;margin:2.5mm 0 1.2mm;border-bottom:.3mm solid #aaa;padding-bottom:.7mm}.head{display:flex;justify-content:space-between;align-items:flex-end}.sub{font-size:7pt;color:#555}.grid{display:grid;grid-template-columns:repeat(6,1fr);gap:1mm}.grid div{border:.2mm solid #ccc;border-radius:1mm;padding:1.2mm}.grid span{display:block;color:#666;font-size:5.8pt}.grid b{display:block;margin-top:.4mm;font-size:7.2pt}.forecast{display:grid;grid-template-columns:repeat(3,1fr);gap:1mm}.forecast>div{border:.2mm solid #aaa;padding:1mm;border-radius:1mm}.forecast p{margin:.5mm 0;font-size:5.5pt;line-height:1.4}.forecast-table{width:100%;border-collapse:collapse;margin-top:1mm;font-size:5.3pt}.forecast-table th,.forecast-table td{border:.2mm solid #bbb;padding:.45mm;text-align:center}.note,.footer{font-size:5.3pt;color:#666;margin-top:1mm}.footer{display:flex;justify-content:space-between}</style></head><body><div class="head"><div><h1>${esc(x.symbol)}</h1><div class="sub">${esc(x.company||'')}</div></div><div class="sub">گزارش روزانه — شکارچی سهم</div></div><h2>آخرین داده روزانه معتبر</h2><div class="grid">${metrics.map(m=>`<div><span>${m[0]}</span><b>${m[1]}</b></div>`).join('')}</div><h2>شش سناریوی تشخیصی ۱۰ روز کاری</h2><div class="forecast">${models.map(m=>`<div><b>${esc(m.name)}</b><p>${esc(m.data.text)}</p></div>`).join('')}</div>${ft}<div class="note">این گزارش بر پایه داده‌های روزانه واقعی تهیه شده است. شاخص‌های لحظه‌ای دفتر سفارش، نقطه ورود و حد ضرر در این حالت در دسترس نیستند و شش سناریوی فوق توصیه قطعی معامله نیستند.</div><div class="footer"><span>تاریخ تهیه: ${printJalaliDateTimeV416(new Date())}</span><span>چاپ / ذخیره PDF</span></div><script>window.onload=()=>setTimeout(()=>window.print(),250)<\/script></body></html>`);
  w.document.close();
  return true;
}
...
printDetail = function(){
  ...
  if(x.analyzed===false){printHistoricalDetailV416(x);return;}
  const models=forecastModels(x),ft=forecastTable(models),w=window.open('','_blank','width=1100,height=1100');if(!w){alert('مرورگر پنجره چاپ را مسدود کرده است. اجازه بازشدن پنجره جدید را برای این سایت فعال کنید.');return;}
  const reasons=decisionReasonsV410(x);
  const metrics=[
