...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    return Response.json({ok:true,scanned:(data||[]).length,candidates:events.length,recorded,shadow_candidates:shadows.length,shadow_recorded:shadowRecorded,trade_date:now.ymd,source_version:'4.1.6-server-v5-carry'});
