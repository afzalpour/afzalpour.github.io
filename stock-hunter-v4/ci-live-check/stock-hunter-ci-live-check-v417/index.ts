...
const EXPECTED_SNAPSHOT_WORKFLOW_REF="afzalpour/afzalpour.github.io/.github/workflows/stock-hunter-live-canonical-snapshot.yml@refs/heads/main";
...
const VERIFIERS={
  ...
  "live-snapshot-v417":{
    workflowRef:EXPECTED_SNAPSHOT_WORKFLOW_REF,
    events:new Set(["push","workflow_dispatch"]),
    sql:LIVE_SNAPSHOT_SQL,
    result:"stock-hunter-live-snapshot-v417: PASS",
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    try{
      ...
      if(verifier.snapshot)response.snapshot=snapshot;
      return out(200,response);
    }catch(e){
