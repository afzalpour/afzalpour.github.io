...
const EXPECTED_PROVENANCE_WORKFLOW_REF="afzalpour/afzalpour.github.io/.github/workflows/stock-hunter-integrated-provenance-recovery-v1.yml@refs/heads/main";
...
`;
const INTEGRATED_PROVENANCE_SQL=`
with target as (
  select c.oid,c.relname,n.nspname
  from pg_class c
  join pg_namespace n on n.oid=c.relnamespace
  where n.nspname='public' and c.relname='stock_hunter_integrated_v1'
),
columns as (
  select a.attnum,a.attname,pg_catalog.format_type(a.atttypid,a.atttypmod) as data_type,
         a.attnotnull
  from pg_attribute a
  where a.attrelid=(select oid from target)
    and a.attnum>0 and not a.attisdropped
  order by a.attnum
),
relation_deps as (
  select distinct rn.nspname as schema_name, rc.relname as object_name, rc.relkind
  from target t
...
function_deps as (
  select distinct pn.nspname as schema_name,p.proname,p.oid
  from target t
  join pg_rewrite rw on rw.ev_class=t.oid
  join pg_depend d on d.classid='pg_rewrite'::regclass and d.objid=rw.oid
...
select jsonb_build_object(
  'captured_at',now(),
  'view','public.stock_hunter_integrated_v1',
  'view_definition',pg_get_viewdef((select oid from target),true),
  'columns',(select coalesce(jsonb_agg(to_jsonb(c) order by c.attnum),'[]'::jsonb) from columns c),
  'relation_dependencies',(
    select coalesce(jsonb_agg(jsonb_build_object(
      'schema',r.schema_name,
      'name',r.object_name,
      'relkind',r.relkind,
      'definition',case when r.relkind in ('v','m') then pg_get_viewdef(format('%I.%I',r.schema_name,r.object_name)::regclass,true) else null end
    ) order by r.schema_name,r.object_name),'[]'::jsonb)
    from relation_deps r
  ),
  'function_dependencies',(
    ...
    ) order by f.schema_name,f.proname),'[]'::jsonb)
    from function_deps f
  )
) as provenance;
`;
...
const VERIFIERS={
  ...
  "integrated-provenance-v410":{
    workflowRef:EXPECTED_PROVENANCE_WORKFLOW_REF,
    events:new Set(["workflow_dispatch"]),
    sql:INTEGRATED_PROVENANCE_SQL,
    snapshotSql:null,
    result:"stock-hunter-integrated-provenance-v410: PASS",
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    try{
      let verifierOutput:unknown=null;
      await sql.begin(async tx=>{
        await tx.unsafe("SET TRANSACTION READ ONLY");
        await tx.unsafe("SET LOCAL statement_timeout = '90000ms'");
        const rows=await tx.unsafe(verifier.sql);
        const row=(rows as any[])?.[0] as any;
        verifierOutput=row?.provenance??null;
      });
