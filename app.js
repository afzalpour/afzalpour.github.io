...
function msgFor(e){
  ...
  const map={
    ...
    'ENTRY_NOT_BALANCED':'سند متوازن نیست.','MIN_TWO_LINES':'حداقل دو ردیف لازم است.','PERIOD_CLOSED':'این تاریخ در یک دوره بسته قرار دارد.',
...
function lineRow(l={}){return `<div class="journal-line" data-line-row><div class="field"><label>حساب</label><select name="account">${accountOptions(l.account_id||'',a=>a.is_active&&a.is_postable)}</select></div><div class="field"><label>طرف‌حساب</label><select name="party">${partyOptions(l.party_id||'')}</select></div><div class="field"><label>بدهکار</label><input name="debit" inputmode="numeric" value="${esc(l.debit&&String(l.debit)!=='0'?l.debit:'')}"></div><div class="field"><label>بستانکار</label><input name="credit" inputmode="numeric" value="${esc(l.credit&&String(l.credit)!=='0'?l.credit:'')}"></div><button type="button" class="danger small" data-remove-line>×</button></div>`}
function bindLines(){document.querySelectorAll('[data-remove-line]').forEach(b=>b.onclick=()=>{if(document.querySelectorAll('[data-line-row]').length<=2)return toast('حداقل دو ردیف لازم است');b.closest('[data-line-row]').remove();updateLineTotals()});document.querySelectorAll('[data-line-row] input').forEach(i=>i.oninput=updateLineTotals);updateLineTotals()}
function updateLineTotals(){const rows=[...document.querySelectorAll('[data-line-row]')],d=rows.reduce((s,r)=>s+bi(r.querySelector('[name=debit]').value),0n),c=rows.reduce((s,r)=>s+bi(r.querySelector('[name=credit]').value),0n);if(Q('lineTotals'))Q('lineTotals').innerHTML=`بدهکار: <b>${money(d)}</b> &nbsp; بستانکار: <b>${money(c)}</b> &nbsp; <span class="${d===c&&d>0n?'pos':'neg'}">${d===c&&d>0n?'متوازن':'نامتوازن'}</span>`}
function journalModal(id=null){
  ...
  Q('journalForm').onsubmit=async ev=>{ev.preventDefault();const f=new FormData(ev.target),rows=[...document.querySelectorAll('[data-line-row]')].map(r=>({account_id:r.querySelector('[name=account]').value,party_id:r.querySelector('[name=party]').value||null,debit:cleanAmount(r.querySelector('[name=debit]').value)||'0',credit:cleanAmount(r.querySelector('[name=credit]').value)||'0'})),d=rows.reduce((s,x)=>s+bi(x.debit),0n),c=rows.reduce((s,x)=>s+bi(x.credit),0n);if(rows.length<2||d<=0n||d!==c)return toast('سند باید حداقل دو ردیف و جمع بدهکار/بستانکار برابر داشته باشد');try{await C.rpc('save_draft_journal',{p_workspace_id:ctx.workspace.id,p_fiscal_year_id:ctx.fiscalYear.id,p_journal_id:e?.id||null,p_entry_date:f.get('date'),p_description:f.get('description'),p_lines:rows});closeModal();await reloadAndRender();toast('پیش‌نویس سند ذخیره شد')}catch(err){showError(err)}};
}
